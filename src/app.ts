    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          {{ t.gettext('Copyright') }} <strong>e-commerce</strong>
          {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
...
export const ECommerceClientRoutes: Routes = [
  {
    ...
    pathMatch: 'full',
    redirectTo: () => {
      if (ECommerceClientRoutes.length === 1) {
        return '';
      }
      return ECommerceClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
export const ECommerceStartFunction = async (
  ...
  await ECommerceContext.initialize(startParams);
